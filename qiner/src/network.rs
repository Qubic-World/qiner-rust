...
use lib::types::{
    Gamma, Nonce, Nonce64, PublicKey64, Signature, NUMBER_OF_NONCE, NUMBER_OF_NONCE_64,
};
...
impl RequestResponseHeader {
    ...
    pub fn get_size(&self) -> usize {
        unsafe { ptr::read_unaligned(&self.size as *const Size as *const usize) }
    }
...
impl Packet {
    pub fn new(r#type: &Type, computor_public_key: &PublicKey64, in_nonce: &Nonce64) -> Self {
        ...
        let header: RequestResponseHeader =
            RequestResponseHeader::new(r#type, &size_of::<Packet>());
        ...
        loop {
            ...
            shared_key_and_gamming_nonce[(gamming_key.len())..]
                .copy_from_slice(nonce_buffer.as_slice());
        ...
        unsafe {
            nonce_buffer
                .iter_mut()
                .zip(nonce_u8_ptr.read().iter())
                .zip(gamma.as_slice())
                .for_each(|((nonce_buffer_value, in_nonce_value), gamma_value)| {
    ...
    pub fn get_random_signature() -> Signature {
        ...
        signature.iter_mut().for_each(|item: &mut u64| unsafe {
            _rdrand64_step(item);
        });
